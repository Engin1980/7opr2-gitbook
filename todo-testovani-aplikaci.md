...
Celý mechanismus je postaven na metodice _Arrange-Act-Assert (AAA)_, která je celosvětově uznávaný standard pro přehlednou strukturu unit testů. Tento vzor rozděluje tělo každé testovací metody do tří jasně oddělených kroků:&#x20;

* Arrange — připravíte všechna potřebná data, objekty a očekávané hodnoty;
* Act — zavoláte samotnou testovanou metodu a zachytíte její reálný výsledek;&#x20;
* Assert — ověříte, zda se reálný výsledek shoduje s tím očekávaným.

Čitelnost a udržitelnost testů se použitím této struktury dramaticky zvyšuje. Když v týmu všichni vývojáři dodržují stejný vzor — navíc vizuálně oddělený prázdnými řádky nebo komentáři —, dokáže kdokoli během několika sekund pochopit, co daný test dělá, aniž by musel složitě luštit jeho logiku.

```java
@Test
void testSpocitaniCenySOdskodnenim() {
    // 1. ARRANGE - Příprava objektů, vstupů a očekávaného výsledku
    KalkulackaCeny kalkulacka = new KalkulackaCeny();
    double zakladniCena = 100.0;
    double slevaProcento = 20.0;
    double ocehavanaCena = 80.0;

    // 2. ACT - Spuštění jedné konkrétní akce, kterou testujeme
    double skutecnaCena = kalkulacka.spocitejCenuPoSleve(zakladniCena, slevaProcento);

    // 3. ASSERT - Porovnání očekávání se skutečností
    assertEquals(ocehavanaCena, skutecnaCena, 0.001, "Cena po 20% slevě ze 100 Kč musí být 80 Kč.");
}
```
...
#### Testování manažera - přidávání aut

Funkce u třídy `CarManager` pro přidávání aut má také několik základních průběhů chování:

* Nová registrace auta validního auta (úspěšný scénáře)
* Odmítnutí registrace `null` auta
* Odmítnutí registrace nevalidního auta, když jeho funkce `isValid()` vrací `true` (tj. nemá barvu nebo registrační značku).
* Odmítnutí registrace auta, které je již zaregistrováno (podle kódu VIN).
...
class CarManagerTest {
    ...
    @Test
    void testRegisterCarOk() {
        // Arrange - Vytvoříme novou instanci manažera a platné auto
        CarManager carManager = new CarManager();
        Car car = new Car("1A2B3C4D5E6F7G8H9", "1AB 1234", Color.RED);

        // Act - Pokusíme se auto zaregistrovat
        boolean actualResult = carManager.registerCar(car);

        // Assert - Registrace musí vrátit true a auto se musí objevit v seznamu
        assertTrue(actualResult, "Registrace platného auta by měla vrátit true.");
        assertEquals(1, carManager.getRegisteredCars().size(), "Seznam registrovaných aut by měl obsahovat 1 auto.");
        assertTrue(carManager.getRegisteredCars().contains(car), "Seznam by měl obsahovat právě vložené auto.");
    }

    @Test
    void testRegisterCarNullCar() {
        // Arrange - Vytvoříme novou instanci manažera a null auto
        CarManager carManager = new CarManager();
        Car car = null;
        ...
        // Assert - Vrátí false a seznam zůstane prázdný
        assertFalse(actualResult, "Registrace null objektu musí vrátit false.");
        assertTrue(carManager.getRegisteredCars().isEmpty(), "Seznam aut musí zůstat prázdný.");
    }

    @Test
    void testRegisterCarInvalidCar() {
        // Arrange - Vytvoříme novou instanci manažera a nevalidní auto (chybí SPZ i barva)
        CarManager carManager = new CarManager();
        Car invalidCar = new Car("1A2B3C4D5E6F7G8H9");

        // Act
        boolean actualResult = carManager.registerCar(invalidCar);

        // Assert - Vrátí false, protože isValid() vrátí false
        assertFalse(actualResult, "Registrace neplatného auta (isValid == false) musí vrátit false.");
        assertTrue(carManager.getRegisteredCars().isEmpty(), "Neplatné auto nesmí být uloženo do seznamu.");
    }

    @Test
    void testRegisterCarDuplicitCar() {
        // Arrange - Vytvoříme novou instanci manažera a zaregistrujeme první auto
        CarManager carManager = new CarManager();
        Car firstCar = new Car("1A2B3C4D5E6F7G8H9", "1AB 1234", Color.RED);
        carManager.registerCar(firstCar);

        // Vytvoříme druhé auto se STEJNÝM VIN, ale s jinou SPZ a barvou
        Car duplicateVinCar = new Car("1A2B3C4D5E6F7G8H9", "9XY 9999", Color.BLUE);

        // Act - Pokusíme se zaregistrovat auto s duplicitním VIN
        boolean actualResult = carManager.registerCar(duplicateVinCar);

        // Assert - Druhá registrace musí selhat
        assertFalse(actualResult, "Registrace auta s již existujícím VIN musí vrátit false.");
        assertEquals(1, carManager.getRegisteredCars().size(), "V seznamu musí zůstat pouze původní auto.");
    }
}
```
