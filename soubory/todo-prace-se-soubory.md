...
| Metoda       | Popis                                                                                                                                     |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------- |
| readAllBytes | Načte obsah souboru jako pole bytů.                                                                                                       |
| write        | Uloží pole bytů do souboru.                                                                                                               |
| readAllLines | Načte ze souboru všechny řádky jako **kolekci řetězců**. Při načítání se použije kódová stránka zadaná jako druhý parametr volání funkce. |
| write        | Druhé přetížení stejné metody umožňuje zapsat do souboru řetězec (či pole řetězců) se zadanou kódovu stránkou.                            |
...
Uložení textu do souboru je obdobně jednoduché. Důležité je však, že i zde se používají funkce využívající proměnného počtu argumentů. Posledním argumentem je volitelně jedna, nebo více následujících hodnot výčtového typu _StandardOpenOptions_, který nabývá mj.[\[47\]](https://word2md.com/#footnote-47) hodnoty uvedené v tabulce.

| Hodnota            | Význam                                                                                                                                                       |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| WRITE              | Otevře soubor pro zápis.                                                                                                                                     |
| APPEND             | Při zápisu se bude přidávat na konec souboru. Používá se v kombinaci s WRITE nebo CREATE.                                                                    |
| TRUNCATE\_EXISTING | Smaže obsah otevíranému souboru (soubor se tedy otevře, smaže se jeho obsah a případný zápis se provádí od začátku souboru). Používá se v kombinaci s WRITE. |
| CREATE\_NEW        | Otevře nový soubor. Pokud již soubor existuje, vyhodí se výjimka.                                                                                            |
| CREATE             | Otevře soubor. Pokud soubor neexistuje, vytvoří se nový.                                                                                                     |
| DELETE\_ON\_CLOSE  | Smaže soubor po ukončení práce. Používá se pro dočasné soubory, které poté programátor nemusí mazat ručně.                                                   |
...
| Směr     | Typ dat | Název        |
| -------- | ------- | ------------ |
| Vstupní  | Byty    | InputStream  |
| Výstupní | Byty    | OutputStream |
| Vstupní  | Znaky   | Reader       |
| Výstupní | Znaky   | Writer       |
...
První tučný řádek provádí otevření souboru. Výpustka (…) bude později nahrazena kódem realizujícím čtení dat. Druhý tučný řádek ukazuje na způsob uzavření souboru.

Ani jeden z tučně zapsaných příkazů však nepůjde přeložit - první příkaz totiž vyhazuje výjimku v případě, že soubor není nalezen. Proto ji ošetříme příkazem try-catch-finally.
...
I tentokrát však kód nepůjde přeložit, protože i zavření souboru může způsobit chybu a vyvolat výjimku. I zde tedy musíme doplnit bloky try-catch. O způsobu, jak korektně ošetřit případ, že se soubor nepodařilo zavřít, lze dlouho polemizovat - v našem případě pouze vypíšeme chybu uživateli na konzoli.
...
Nyní lze vidět, že již samotné otevření a uzavření soubor má poměrně složitou režii. Samozřejmě, případ lze již řešit pomocí varianty _try-with-resources_ představené výše. Následuje doplnění kódu pro čtení ze souboru. Při čtení ze souboru se používá funkce _read()_, která v případě, že již nejsou žádná další data k dispozici, vrací hodnotu -1.
...
Pro úplnost je doplněn příklad využívající _try-with-resource_ konstrukci.
...
Je vhodné si povšimnout, že zcela zmizel blok _finally_, protože blok _try-with-resource_ se o uzavření zdroje postará sám. Naplnění proměnné _bufferedReader_ se musí provést v rámci jednoho příkazu, proto se uvnitř konstruktoru třídy _BufferedReader_ volá jako parametr konstruktor třídy _FileReader_ s názvem souboru. Funkčnost obou příkladů je identická.
...
| Název třídy                               | Použití                                                                                                                                                                                                                                        |
| ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ByteArrayInputStreamByteArrayOutputStream | Proudy pro čtení/zápis do pole bytů. Programátor tak nemusí k poli bytů přistupovat pomocí indexů, ale vytvoří si polem proud a jednotlivé byty čte nebo zapisuje postupně.                                                                    |
| PushbackInputStreamPushbackReader         | Přidává funkcionalitu čtení dalšího znaku bez jeho výběru. Nad vnořeným streamem tedy umí provést operaci, kdy se „podívá", co je další znak.                                                                                                  |
...
| PrintStreamPrintWriter                    | Alternativa k proudu OutputStream. Příkladem může být _System.out_. Umí lépe (smysluplněji) vypisovat metodou _write()_ jiné datové typy; hlavně však u těchto operaci nevyhazuje výjimky, takže se výpisy nemusí balit do sekvence try-catch. |
| CharArrayReaderCharArrayWriter            | Obdoba typu _ByteArrayInputStream_, podkladovými daty je však pole znaků (nikoliv bytů).                                                                                                                                                       |
...
Další trojpříkaz zapíše do proudu počet položek v kolekci (_this.size()_) a následně připíše znak konce řádku. **Pozor na zápis čísla do proudu!** Metoda _write()_ má přetížení přijímající jako parametr _int_, ale zapisuje jej jako char (tj. podle ASCII tabulky nebo kódování, místo 60 tedy napíše písmeno „A" apod.). Cílem je však zapsat řetězec reprezentující dané číslo, proto musíme hodnotu _int_ převést na řetězec a teprve tento řetězec zapsat do souboru. O převod čísla na řetězec se nám postará metoda _toString()_ třídy _Integer_ - celý příkaz tedy je _Integer.toString(\<hodnota\_k\_prevedeni>)_. Poslední příkaz zajistí propsání zadaných dat do cílového zdroje pomocí metody _flush()_.
...
&#x20;public static PersonCollection load (Reader reader)
...
Nyní již stačí do třídy _PersonCollection_ dopsat metodu na uložení dat _serialize()_ s využitím třídy _ObjectOutputStream_.
...
sr eng.demos.ionio.Personź\_âŽ.x]ú I ageL namet Ljava/lang/String;L phonest Ljava/util/List;xp ­t Michalsq \~ w
...
* Lze pomocí proudu načítat textový soubor? Pokud ano, proč existuje třída _Files?_
* Popište princip zapisovacího proudu.
* Popište princip serializace.

Úkoly k zamyšlení:

Kde všude lze techniku proudů ještě použít?

Korespondenční úkol:

Vytvořte proudy, které umí provádět RLE kompresi a dekompresi. Výsledek demonstrujte.

Shrnutí obsahu kapitoly

Kapitola představila problematiku načítání a zapisování do souborů. Byly představeny proudy jako obecný mechanismus pro práci se soubory. Část kapitoly byla věnována automatizovanému mechanismu pro ukládání a načítání objektů nazvanému serializace.
