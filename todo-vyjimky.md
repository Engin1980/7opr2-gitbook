...
* Jakým způsobem zpracovává jazyk Java chybové stavy?
* Co jsou výjimky?
* Do jakých skupin se dělí?

Po jejím prostudování byste měli být schopni:

* Zachytit libovolnou chybu pomocí výjimky
* Vytvořit vlastní výjimku
* Umět vyvolat libovolnou výjimku

Klíčová slova této kapitoly:

Výjimky, try-catch-finally, zřetězení výjimek

Doba potřebná ke studiu: 6 hodin

**Průvodce studiem**

Kapitola představuje problematiku práce s chybami v programovacím jazyce Java. Jsou představeny výjimky, a jakým způsobem je lze vyvolávat a zachytávat, jak pomocí nich ošetřit a naopak zotavovat chybové stavy aplikace. Je představen pojem zřetězení výjimek a problematika povinnosti zpracování výjimky.

Pro studium kapitoly a samostatné ověření příkladů vlastními programy si vyhraďte 6 hodin.
...
Čtvrtý blok zachycuje obecně všechny výjimky, jaké mohou v programu nastat.
...
try (datový\_typ\_zdroje proměnná = inicializace) { /\* obsah bloku \*/ }
...
* Co je to výjimka z pohledu jazyka Java?
* Co jsou „chained exceptions"?
* Co jsou „checked exceptions"?
* Jak je to s povinností výskytu jednotlivých bloků try-catch-finally?
* Jak zjistit typ výjimky a bližší informace o ní?
* Jak vyvolat výjimku?

Úkoly k zamyšlení:

Proč je mechanismus pro práci se zdroji při zpracování výjimek řešen jiným způsobem?

Korespondenční úkol:

Vytvořte vlastní běhovou výjimku, kterou můžete vyvolávat při libovolné chybě aplikace - tedy když „programátor něco naprogramoval špatně". Demonstrujte použití.

Shrnutí obsahu kapitoly

Kapitola představuje problematiku zachytávání chybových stavů. Jsou představeny výjimky, jejich dělení a hierarchie v rámci dědičnosti, problematika vyvolání, zachycení, zpracování výjimky a zotavení po chybě. Je představen způsob, jak vytvořit vlastní výjimku, jak výjimky zřetězit a jak pracovat s výjimkami v případě práce s datovými zdroji, které je třeba uzavírat automaticky.
